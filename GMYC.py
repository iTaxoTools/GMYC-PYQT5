...
class um_tree:
    def __init__(self, tree, fileextension=None, unique=None):
    ...
    def print_species(self, save_file):
        cnt = 1
        file3= open(os.path.join(save_file,  self.unique+ "partition.txt"), "w+")
        for sp in self.species_list:
    ...
    def print_species_spart(self, save_file, p_value=None):

        fo_bestpar = open(os.path.join(save_file, self.unique + "partition.spart"), "w+")
        ...
        fo_bestpar.write(f'Date = {datetime.datetime.now().astimezone().isoformat()};\n')
        fo_bestpar.write(f"N_spartitions = 1 : {file}_GMYC, {p_value};\n")

        fo_bestpar.write(f'N_individuals = {sum(len(sp) for sp in self.species_list)};\n')
        fo_bestpar.write(f'N_subsets = {len(self.species_list)};\n')
        fo_bestpar.write("[Generated by GMYC]\n")
        fo_bestpar.write("[WARNING: The sample names below may have been changed to fit SPART specification (only alphanumeric characters and _ )]\n")
        fo_bestpar.write("Individual_assignment = \n")
        ll=[]
        ...
        fo_bestpar.write(f"Spartition_score_type = Likelihood ratio test p-value;\n")
    ...
    def num_lineages(self, wt_list, save_file):
        ...
        plt.xlabel('Time')
        plt.savefig(os.path.join(save_file, self.unique+ "Time_Lines.png"))
...
class waiting_time:
    ...
    def logl(self):
        ...
        if prob <=0:
            print("branch length:" + repr(self.length))
            print("b:" + repr(self.b))
            return None
        else:
...
def optimize_null_model(umtree):
    ...
    while change > min_change and cnt < max_iters:
        cnt = cnt + 1
        para, nn, cc = fmin_l_bfgs_b(tar_fun_null, [1], args = tuple([nm]), bounds = [[0, 10]], approx_grad = True)
...
def gmyc(tree, print_detail = False, show_tree = False, show_llh = False, show_lineages = False, print_species = False, print_species_spart = False, pv = 0.01, save_file= "."):
    llh_list = []
    min_change = 0.1
    max_iters = 100
    best_llh = float("-inf")
...
def gmyc_func(tree, taxa_order, print_detail = False, show_tree = False, show_llh = False, show_lineages = True, print_species = True, print_species_spart = True, pv = 0.01):
    llh_list = []
    min_change = 0.1
    max_iters = 100
    best_llh = float("-inf")
    ...
    for tnode in utree.nodes:
        ...
        while change > min_change and cnt < max_iters:
            cnt = cnt + 1
            para, nn, cc = fmin_l_bfgs_b(tar_fun, [1, 1], args = tuple(tt), bounds = [[0, 10], [0, 10]], approx_grad = True)
            #para, nn, cc = fmin_tnc(tar_fun, [0, 0], args = tuple(tt), disp = False, bounds = [[0, 10], [0, 10]], approx_grad = True)
            tt.update(para[0], para[1])
            logl = tt.sum_llh()
            change = abs(logl - last_llh)
            last_llh = logl
